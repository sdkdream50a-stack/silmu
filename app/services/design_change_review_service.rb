...
class DesignChangeReviewService
  ...
  CHANGE_REASONS = {
    site_condition: {
      ...
      legal_basis: "지방계약법 시행령 제74조(설계 변경으로 인한 계약금액의 조정) + 행안부 예규 「지방자치단체 공사계약 일반조건」",
      legal_text: "공사현장의 상태가 설계서와 다른 경우 (일반조건상 설계 변경 사유)",
      documents: [
        "현장 상태 확인서 (사진 포함)",
        "당초 설계도서",
        "현장 측량 성과",
    ...
    design_error: {
      ...
      legal_basis: "지방계약법 시행령 제74조 + 행안부 예규 「지방자치단체 공사계약 일반조건」",
      legal_text: "설계서의 내용이 불분명하거나 누락·오류가 있는 경우 (일반조건상 설계 변경 사유)",
      documents: [
        "설계오류 확인서",
        "당초 설계도서 (오류 부분 표시)",
        "설계변경 사유서",
    ...
    civil_request: {
      ...
      legal_basis: "지방계약법 시행령 제74조 + 행안부 예규 「지방자치단체 공사계약 일반조건」",
      legal_text: "기타 발주기관이 설계변경이 필요하다고 인정하는 경우 (일반조건상 설계 변경 사유)",
      documents: [
        "민원 접수서 (신청서·진정서 등)",
        "민원 검토 의견서",
        "설계변경 사유서",
    ...
    quantity_change: {
      ...
      legal_basis: "지방계약법 시행령 제74조 제1항(공사량 증감 시 계약금액 조정)",
      legal_text: "공사량의 증감이 발생한 경우 — 시행령 §74①항 본문에 해당",
      documents: [
        "당초 수량산출서",
        "변경 수량산출서",
        "실측 자료 (사진·도면)",
    ...
    method_change: {
      ...
      legal_basis: "지방계약법 시행령 제74조 제5항(새로운 기술·공법 등에 따른 설계 변경)",
      legal_text: "새로운 기술·공법 사용으로 공사비 절감 등 효과가 현저한 경우 — 절감액의 30%를 감액하여 계약금액 조정",
      documents: [
        "공법 변경 검토서",
        "당초 공법 대비 비교표",
        "변경 공법 기술자료",
