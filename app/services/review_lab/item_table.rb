...
module ReviewLab
  ...
  class ItemTable
    ...
    # ⚠️ 「총계」는 반드시 여기 있어야 한다(독립 검토 R3 · 공공 서식 실측). `FieldExtractor` 는
    #    「총계」를 grand_total 라벨로 **이미** 인정하는데 여기에 없으면 같은 행을 한쪽은 «합계»로,
    #    다른 쪽은 «품목»으로 읽어 금액이 이중으로 더해지고 **정상 문서에 거짓 BLOCK** 이 난다.
    STOP_ROW = /\A(?:합\s*계|계|공급가액(?:\s*합계)?|부가세|부가가치세|세액|총\s*액|총\s*계|합계금액|총\s*견적금액)\s*(?:\([^)]{0,20}\))?\s*(?:[:：|]|\s+\d|\z)/
