  ...
  class PackageReviewer
    ...
    def build_matrix
      ...
      (COMPARE.keys + MATRIX_ONLY).each do |key|
        ...
        # 다품목 문서는 대조에서만 빼고, 수량이 하나뿐인 문서끼리는 계속 대조한다(한 문서 때문에 전체를 끄지 않는다).
        present = present.reject { |label, _f| multi_docs.any? { |d| d.label == label } } if multi_docs.any?
        multi_qty = multi_docs.any? && present.size < 2
        ...
        verdict = if multi_qty then "원문 대조"
        elsif MATRIX_ONLY.include?(key) then present.size >= 2 ? "원문 대조" : "단일"
