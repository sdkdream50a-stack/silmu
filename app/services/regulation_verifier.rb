...
class RegulationVerifier
  ...
  TOOL_VERIFICATIONS = {
    ...
    "estimated_price" => {
      ...
      checks: [
        "수의계약 한도: 물품/용역 2천만원, 공사 종합 4억/전문 2억/기타 1.6억원",
        "견적 요건: 2백만원 이하 생략 가능, 2천만원 이하 1인 견적",
        "이윤율 상한: 시설공사 15% (회계예규 §21, 노무비+경비+일반관리비 기준, 기술료·외주가공비 제외)",
        "일반관리비 상한: 시설공사 일반건설공사 50억 미만 6.0% / 50~300억 5.5% / 300억 이상 5.0% (회계예규 §20, 재료비+노무비+경비 기준)"
      ]
    },
    "contract_reason" => {
    ...
    "legal_period" => {
      ...
      checks: [ "입찰공고(공사·현장설명 미실시): 10억 미만 7일, 10억~50억 15일, 50억~고시금액 30일, 고시금액 이상 40일" ]
    }
  }.freeze
  ...
  def build_verification_prompt(topic_name, field, content, keywords)
    <<~PROMPT
      ...
      - 물품·용역의 분할 조달: 시행령 제7조제2호(추정가격 합산 — 직전/직후 12개월 또는 해당 회계연도 총액)
        + 행안부 예규 「지방자치단체 입찰 및 계약 집행기준」 제1장 제1절 5.라 (단일 사업의 **부당한** 분할 금지 · 도서 등 불가피한 경우 예외)
      ...
      **수의계약 한도** (시행령 제25조):
      ...
      - 공사 - 전문공사: 2억원 이하 ✅
      - 공사 - 기타공사: 1.6억원 이하 ✅
      - 물품/용역 - 일반: 2천만원 이하 ✅ (제25조제1항제5호나목)
      - 물품/용역 - 청년창업기업: 2천만원 초과 5천만원 이하 ✅ (같은 호 다목)
      - 물품/용역 - 소기업·소상공인: 2천만원 초과 1억원 이하 ✅ (같은 호 라목)
      - 물품/용역 - 여성·장애인·사회적기업·사회적협동조합·자활기업·마을기업: 2천만원 초과 1억원 이하 ✅ (같은 호 바목)

      **1인/2인 견적 기준** (시행령 제30조):
      - 1인 견적: 2천만원 이하 (일반), 5천만원 이하 (특례 — 제30조제1항제2호 가목 청년창업·여성·장애인 / 나목 사회적기업·사회적협동조합·자활기업·마을기업) ✅
        ※ §30 의 5천만원 특례 열거는 §25 의 1억원 특례 대상과 다르다. 두 축을 합치면 반드시 틀린다
      - 2인 이상 견적: 2천만원 초과 ✅

      **입찰 관련 기준**:
      - 입찰공고 기간(공사·현장설명 미실시, 시행령 제35조제3항): 10억 미만(7일), 10억~50억(15일), 50억~고시금액(30일), 고시금액 이상(40일) ✅
      - 복수예비가격: 2억원 이상 ✅
