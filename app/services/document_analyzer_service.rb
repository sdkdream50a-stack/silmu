...
class DocumentAnalyzerService
  ...
  def build_quote_extraction_prompt(extracted_text = nil)
    ...
    <<~PROMPT
      ...
      - unit: 단위 (예: "개", "EA", "식", "m²")
      - qty: 수량 (숫자)
      - unit_price: 단가 (숫자, 원 단위). 반드시 문서에 적힌 단가를 그대로 사용하세요.

      ### 금액
      - supply_amount: 공급가액 (VAT 제외, 숫자, 원 단위). 반드시 문서에 적힌 공급가액을 그대로 사용하세요.
      - vat_amount: 부가세 (숫자, 원 단위). 반드시 문서에 적힌 부가세를 그대로 사용하세요.
      - total_amount: 합계금액 (VAT 포함, 숫자, 원 단위). 반드시 문서에 적힌 합계금액을 그대로 사용하세요.
      ...
      4. **금액 정확성이 최우선입니다.** supply_amount, vat_amount, total_amount는 반드시 문서에 표기된 숫자를 그대로 사용하세요. 절대 직접 계산하거나 항목을 합산하지 마세요.
      5. items에는 개별 실물 품목/공종만 포함. 다음 행은 반드시 제외하세요:
         - 소계, 합계, 계, 부가세, 부가가치세 행
         - 간접공사비, 일반관리비, 이윤, 산업안전보건관리비, 산재보험료, 고용보험료, 환경보전비, 제경비, 기술료 등 간접비 항목
         - 공사 견적서의 경우: 재료비·노무비·경비가 별도 행이면 합산된 공종 단위로만 추출
      6. **중복 추출 금지**: 동일한 비용이 항목(items)과 간접비에 이중으로 포함되지 않도록 주의하세요.
         - items의 unit_price는 해당 품목의 단가만 포함 (간접비·이윤 미포함)
         - 간접비(일반관리비, 이윤 등)는 items에 넣지 말고, supply_amount에 자연스럽게 포함되도록 하세요
      7. 수량과 단가를 알 수 없으면 qty: 0, unit_price: 0으로 표기하세요.
      8. project_name이 없으면 품목들을 종합하여 간단히 요약하세요.
      9. 반드시 JSON 형식으로만 응답하세요.

      ## 공사 견적서 주의사항
      공사 견적서는 다음과 같은 구조를 가집니다:
      - 직접공사비: 개별 공종(품명)별 재료비+노무비+경비 합산 단가
      - 간접공사비: 일반관리비, 이윤, 산재보험료, 안전관리비 등
      - 공급가액 = 직접공사비 + 간접공사비
      - 부가세 = 공급가액 × 10%
      - 합계 = 공급가액 + 부가세

      items에는 직접공사비의 개별 공종만 포함하세요. 간접공사비 항목은 items에 포함하지 마세요.
      items 합계가 supply_amount보다 작은 것이 정상입니다(차이 = 간접공사비).
