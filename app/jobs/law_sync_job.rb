...
class LawSyncJob < ApplicationJob
  ...
  TARGET_LAWS = [
    ...
    "지방재정법 시행령",
    "소득세법",
    # 2026-05-17 — 매년 갱신 단가 드리프트 대응 (인사혁신처 1월 일괄 개정 감지)
    "공무원보수규정",
    "공무원수당 등에 관한 규정",
    "공무원연금법",
    "공무원연금법 시행령",
    # 2026-06-09 S5 — 청탁금지법 추가. 시행령 별표1(음식물·선물·경조사비 한도) 개정은 시행일 변경을 동반하므로
    # 기존 effective_date 감지로 별표 금액 개정(예: 2024-08 음식물 3만→5만)을 자동 포착 → 1.5년 누락 재발 방지
