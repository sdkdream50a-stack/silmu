class GuidesController < ApplicationController
  GUIDES = {
    1 => {
      ...
      sections: {
        ...
        laws: ["지방자치단체를 당사자로 하는 계약에 관한 법률", "지방자치단체를 당사자로 하는 계약에 관한 법률 시행령", "지방자치단체 물품관리 조례", "지방자치단체 입찰 및 계약 집행기준"]
      }
    },
    2 => {
      id: 2,
      ...
      sections: {
        ...
        laws: ["지방자치단체를 당사자로 하는 계약에 관한 법률 시행령 제64조", "지방자치단체 물품관리 조례", "물품관리법"]
      }
    },
    3 => {
      id: 3,
      ...
      views: 58,
      author: "실무팀",
      description: "계약 전 적정한 예정가격을 산정하는 방법과 절차를 안내합니다.",
      sections: {
        requirements: [
          { title: "예산 확인", desc: "배정된 예산 범위를 확인합니다." },
          { title: "물품 규격 확정", desc: "구매할 물품의 정확한 규격을 확정합니다." },
          { title: "시장조사 계획", desc: "가격 조사 방법과 대상을 계획합니다." },
          { title: "조사 기간 확보", desc: "충분한 시장조사 기간을 확보합니다." }
        ],
        step1: { title: "시장조사", items: ["나라장터 종합쇼핑몰에서 동일·유사 물품 가격을 조사합니다.", "3개 이상 업체로부터 견적서를 수집합니다.", "인터넷 쇼핑몰, 카탈로그 등 다양한 경로로 가격을 조사합니다."] },
        step2: { title: "가격 분석", items: ["수집된 가격 자료를 비교 분석합니다.", "극단적으로 높거나 낮은 가격은 제외합니다.", "적정 가격 범위를 산정합니다."] },
        step3: { title: "예정가격 결정", items: ["시장조사 결과를 바탕으로 예정가격을 산정합니다.", "예정가격조서를 작성합니다.", "예정가격 결정 사유를 명확히 기재합니다.", "결재권자의 승인을 받습니다."] },
        step4: { title: "예정가격 관리", items: ["예정가격은 입찰 전까지 비공개로 관리합니다.", "예정가격조서는 별도로 봉인 보관합니다.", "개찰 시까지 비밀을 유지합니다."] },
        cautions: ["예정가격을 사전에 누설하면 형사처벌 대상입니다.", "특정 업체에 유리하게 예정가격을 조작하면 안 됩니다.", "시장조사 없이 임의로 예정가격을 정하면 안 됩니다.", "예정가격조서 작성일자를 소급하면 안 됩니다."],
        laws: ["지방자치단체를 당사자로 하는 계약에 관한 법률 시행령 제7조", "지방자치단체 입찰 및 계약 집행기준", "예정가격 작성기준"]
      }
    },
    4 => {
      id: 4,
      ...
      sections: {
        ...
        laws: ["지방자치단체를 당사자로 하는 계약에 관한 법률 제9조", "지방자치단체를 당사자로 하는 계약에 관한 법률 시행령 제25조", "지방자치단체 입찰 및 계약 집행기준"]
      }
    },
    5 => {
      id: 5,
      ...
      sections: {
        ...
        laws: ["공무원 여비 규정", "지방공무원 여비 업무 처리기준", "각 지방자치단체 여비 조례"]
      }
    },
    6 => {
      id: 6,
      ...
      sections: {
        ...
        laws: ["국가공무원 복무규정", "지방공무원 복무규정", "각 지방자치단체 복무 조례"]
      }
    },
    7 => {
      id: 7,
      ...
      sections: {
        ...
        laws: ["민원 처리에 관한 법률", "민원 처리에 관한 법률 시행령", "각 지방자치단체 민원 조례"]
      }
    },
    8 => {
      id: 8,
      ...
      sections: {
        ...
        laws: ["지방재정법", "지방재정법 시행령", "지방자치단체 예산편성 운영기준"]
      }
    }
  }.freeze
  ...
  def index
    set_meta_tags(
      ...
      og: { title: "업무 가이드 — 실무.kr", url: request.original_url }
    )
  end
  ...
  def show
    ...
    set_meta_tags(
      ...
      og: { title: @guide[:title], description: @guide[:description].truncate(200), url: request.original_url }
    )
  end
  ...
  def contract_flow
    set_meta_tags(
      ...
      og: { title: "계약 흐름도 — 실무.kr", url: request.original_url }
    )
  end
  ...
  def pre_contract_checklist
    set_meta_tags(
      ...
      og: { title: "계약업무 사전 체크리스트 — 실무.kr", url: request.original_url }
    )
  end
  ...
  def resources
    set_meta_tags(
      ...
      og: { title: "자료실 — 실무.kr", url: request.original_url }
    )
    @resources = [
