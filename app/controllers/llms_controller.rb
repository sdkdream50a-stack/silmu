class LlmsController < ApplicationController
  ...
  TOOLS = [
    ...
    [ "qualification-evaluation", "적격심사 입찰률 확인", "낙찰하한율 미달 여부와 입찰률을 확인합니다. 가격평점은 예규 별표의 금액구간별 산식이라 이 도구에서 산출하지 않으며, 적격 여부·낙찰자는 공고문 기준으로 판단해야 합니다" ],
  ...
  def build_summary
    ...
    out << ""
    out << "Q: 대금지급 기한은?"
    out << "A: 검사 완료 후 청구를 받은 날부터 5일 이내 지급해야 하며, 지연 시 지연이자를 지급해야 합니다(지방계약법 제18조, 시행령 제67조)."
    out << ""
