class Exam::ChaptersController < ApplicationController
  ...
  def show
    ...
    # 챕터 미니 퀴즈 (최대 5문제, 사전 슬라이싱+난이도 포함 데이터 사용)
    @mini_questions = ExamQuestions.sliced_by_chapter(@subject[:id], @chapter[:number]).first(5)
    ...
    set_meta_tags(
      ...
      description: "공공조달관리사 #{@subject[:number]} #{@subject[:title]} — 제#{@chapter[:number]}장 #{@chapter[:title]}. 학습목표·핵심키워드·출제 예상 포인트(실무.kr 예측) 정리.",
