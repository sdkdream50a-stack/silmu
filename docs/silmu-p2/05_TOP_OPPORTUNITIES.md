...
> **2026-09-06 R1 정정.** 처음 적은 `#1 #2 #4 #6 #7 #12 #13 #14` 는 **틀렸다.**
> 로컬 실측 결과 `#4 #6 #7 #12 #13 #14` 는 **이미 도구가 매칭되고 있었다**(테마 tool ≥ 1).
> 실제 미매칭이면서 **도구가 그 질의를 실제로 푸는** 것은 질의 단위 8개 형태였고,
> R1 에서 5개 도구 · 7개 낱말로 전부 해소했다(음성대조 불변).
> 나머지는 `TOOL_MISSING` — 도구 자체가 없어 keyword 로 해소되지 않는다.
> 상세 = `12_R1_DATA_INTEGRITY_AND_DISCOVERABILITY.md` §4.

기존 39개 도구 중 일부가 질의에 매칭되지 않는다.
