...
attrs = {
  ...
  law_content: <<~CONTENT,
    ...
    **경조사 특별휴가 (주요 항목):**
    ...
    | 출산 (배우자) | 10일 |
    | 사망 — 배우자, 부모, 자녀 | 5일 |
    | 사망 — 조부모, 외조부모, 배우자 부모 | 3일 |
    | 사망 — 형제·자매 | 1일 |
    | 자녀 결혼 | 1일 |
    | 입양 | 20일 |

    **출산 특별휴가:**
    - 출산한 여성 공무원: 출산 전·후 90일 (다태아 120일)
    - 배우자 출산: 10일 (의무 사용)
    </div>
  CONTENT
  decree_content: <<~CONTENT,
    ...
    **특별휴가 유형별 정리**
    ...
    | 특수업무 | 위험직종 종사자 | 기관별 별도 규정 |
    </div>

    **경조사 특별휴가 신청 절차**

    1. 경조사 특별휴가 신청서 작성
    2. 증빙서류 첨부 (혼인관계증명서, 사망진단서, 출생신고서 등)
    3. 소속 기관장 승인
    4. 사용 후 결과 기록 (e-사람 입력)
  ...
  practical_tips: <<~CONTENT,
    ...
    **놓치기 쉬운 특별휴가**
    ...
    - 사망 특별휴가 — 형제자매 사망 시 1일이지만 놓치기 쉬움
  ...
  qa_content: <<~CONTENT,
    ...
    **Q. 조부모 사망 특별휴가는 외조부모도 해당되나요?**
    A. 네, 조부모와 외조부모 모두 3일의 특별휴가 대상입니다. 또한 배우자의 부모(시부모·장인·장모) 사망도 3일의 특별휴가를 사용할 수 있습니다.
