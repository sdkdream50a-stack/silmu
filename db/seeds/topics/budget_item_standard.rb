...
attrs = {
  ...
  law_content: <<~CONTENT,
    <strong>지방재정법 제41조 (예산의 내용)</strong>

    <div style="background:#dbeafe;padding:12px;border-radius:8px;margin:8px 0;">
    지방재정법 제41조는 지방자치단체의 세입·세출예산, 계속비, 채무부담행위 등 예산의 내용을 규정합니다.

    ■ 세출예산 과목 구분은 같은 법 시행령으로 위임
    - 지방재정법 제41조 제2항: "예산의 과목 구분은 대통령령으로 정한다"
    - 이에 따라 시행령 제46조에서 세출예산의 장·관·항·세항·목 체계를 규정
    </div>

    <div style="background:#eff6ff;padding:12px;border-radius:8px;margin:8px 0;">
    위임 체계: 지방재정법 제41조(예산의 내용) -> 같은 법 시행령 제46조(세출예산의 과목 구분) 및 별표 1 -> 행정안전부 "지방자치단체 예산편성 운영기준"(훈령)에서 세부 집행기준 규정

    ■ 목(目) 간 이동은 지방재정법 제47조(예산의 전용)에 따른 "전용" 절차 필요
    </div>

    <div style="background:#f3f4f6;padding:12px;border-radius:8px;margin:8px 0;">
    관련 법령: 지방재정법 제41조(예산의 내용), 제47조(예산의 전용) / 같은 법 시행령 제46조(세출예산의 과목 구분), 별표 1
    </div>
  CONTENT
  decree_content: <<~CONTENT,
    ...
    <div style="background:#dbeafe;padding:12px;border-radius:8px;margin:8px 0;">
    세출예산의 과목은 장(章) - 관(款) - 항(項) - 세항(細項) - 목(目)으로 구분하며, 구체적 구분은 같은 시행령 별표 1에서 정합니다.

    예시: 일반공공행정(장) -> 일반행정(관) -> 일반행정(항) -> 일반행정관리(세항) -> 업무추진비(목)
    </div>
