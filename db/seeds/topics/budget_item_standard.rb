...
attrs = {
  ...
  law_content: <<~CONTENT,
    <strong>지방재정법 제41조 (예산의 과목 구분)</strong>

    <div style="background:#dbeafe;padding:12px;border-radius:8px;margin:8px 0;">
    지방재정법 제41조는 지방자치단체 세입예산(장·관·항)과 세출예산(분야·부문·정책사업·단위사업·세부사업·목)의 과목 구분 체계를 규정합니다.

    ■ 세출예산 과목 구분 운용은 같은 법 시행령으로 위임
    ...
    위임 체계: 지방재정법 제41조(예산의 과목 구분) → 같은 법 시행령 제47조(예산의 과목구분 및 설정) → 행정안전부 「지방자치단체 예산편성 운영기준」에서 세부 집행기준 규정

    ■ 목(目) 간 이동: 지방재정법 제47조는 예산의 <strong>목적 외 사용을 금지</strong>하며, 정책사업 내 단위사업·목 간 이동은 지방재정법 <strong>제49조(예산의 전용)</strong>에 따른 절차를 거쳐야 합니다.
    </div>

    <div style="background:#f3f4f6;padding:12px;border-radius:8px;margin:8px 0;">
    관련 법령: 지방재정법 제41조(예산의 과목 구분), 제47조(목적 외 사용금지), 제49조(예산의 전용) / 같은 법 시행령 제47조(예산의 과목구분 및 설정)
    </div>
  CONTENT
  decree_content: <<~CONTENT,
    ...
    <div style="background:#dbeafe;padding:12px;border-radius:8px;margin:8px 0;">
    ① 세입예산의 과목은 세입의 원천을 고려하여 지방세수입·세외수입·지방교부세·조정교부금·보조금·지방채 및 예치금회수 등으로 구분합니다.

    ② 세출예산의 과목은 그 기능을 고려하여 일반공공행정·공공질서 및 안전·교육·문화 및 관광·환경보호·사회복지·보건·농림해양수산 등으로 구분합니다.

    ③ 세입·세출 과목구분과 설정에 관한 세부사항은 행정안전부장관이 정합니다(예: 「지방자치단체 예산편성 운영기준」).
    </div>
  ...
  rule_content: <<~CONTENT,
    ...
    **세출예산 과목 구조 (행안부 훈령 기준)**

    <div style="background:#eff6ff;padding:12px;border-radius:8px;margin:8px 0;">
    장(章) -> 관(款) -> 항(項) -> 세항(細項) -> 목(目)

    ■ "목(目)"이 실제 집행의 기본 단위로, 같은 세항이라도 목이 다르면 전용 절차 없이 집행 불가
    ■ 세부 목별 집행 범위는 "지방자치단체 세출예산 집행기준"(행안부)에서 구체적으로 규정
    </div>

    ## 업무추진비 집행 기준

    **업무추진비 사용 기준 (행안부 세출예산 집행기준)**
