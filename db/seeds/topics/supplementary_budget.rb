...
attrs = {
  ...
  law_content: <<~CONTENT,
    ...
    **지방재정법 제45조 (추가경정예산)**
    ...
    ① 지방자치단체의 장은 예산 성립 후 다음 각 호의 어느 하나에 해당하는 사유가 발생하면 추가경정예산안을 편성할 수 있다.

    1. 국가 시책의 변경, 국가의 예산 변경 또는 재원의 변경
    2. 재해 예방 또는 복구를 위한 긴급 지출
    3. 법령상 의무적으로 지출해야 하는 경비 증가
    4. 업무량의 변경으로 인한 경비의 증감
    5. 예산 성립 후 발생한 불가피한 사유에 따른 경비 증감

    ② 추가경정예산안은 본예산안 심의에 준하여 지방의회의 의결을 거쳐야 한다.
    </div>

    <div style="background:#f3f4f6;padding:12px;border-radius:8px;margin:8px 0;">
    ※ 추경 vs 당초예산
    - 당초예산: 회계연도 전에 편성, 의회 의결로 확정 (11~12월)
    - 추경예산: 회계연도 중 필요에 따라 수정·변경
    - 당해 연도 중 추경 횟수 제한 없음 (보통 1~3차 편성)
    </div>
  CONTENT
  decree_content: <<~CONTENT,
    ...
    **지방의회 의안 처리 기간**
    ...
    | 구분 | 처리 기간 |
    |------|----------|
    | 임시회 소집 | 단체장 요구 후 15일 이내 소집 |
    | 예산 심의 기간 | 제출일로부터 20일 이내 의결 (상임위+본회의) |
    | 긴급 처리 | 천재지변 등 긴급 시 단축 가능 |
    </div>
  CONTENT
