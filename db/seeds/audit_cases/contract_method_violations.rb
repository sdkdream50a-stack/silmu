...
지방계약법 시행령 제25조에 따라 1인 견적(소액 수의계약) 방식이 허용되는 물품 구매 한도는 **2천만원 이하**입니다. 추정가격 2천만원 이하에서는 1인 견적이 허용되고(시행령 제30조제1항제2호, 청년창업·여성·장애인기업 등은 5천만원 이하), 그 초과는 원칙적으로 2인 이상 견적 또는 경쟁입찰 대상입니다.
...
- **물품·용역**: 2천만원 이하 (여성·장애인·사회적기업·소기업·청년창업기업 등 특례 시 5천만원~1억원 이하)
- **공사**: 종합 4억원 이하, 전문 2억원 이하, 기타 1억 6천만원 이하
- **견적 방법**: 추정가격 2천만원 이하는 1인 견적, 2천만원을 초과하면 2인 이상 견적으로 처리(공사·물품·용역 공통)
...
물품·용역 기준 수의계약 금액 한도는 **2천만원 이하**이며, 이 범위에서는 **1인 견적(단독 견적)**도 허용됩니다(시행령 제30조제1항제2호). 청년창업·여성·장애인기업 등은 5천만원 이하까지 1인 견적이 가능합니다.
...
지방계약법 제9조는 경쟁입찰 원칙을 규정하며, 분할계약은 공사의 경우 시행령 제77조가, 물품·용역의 경우 시행령 제7조제2호(추정가격 합산)와 행정안전부 예규(집행기준 제1장 제1절 5.라)가 규율합니다. "관행이었다", "바쁘다"는 이유는 면책 사유가 아닙니다.
...
AuditCase.find_or_create_by!(slug: 'spec-price-integration-violation') do |ac|
  ...
  ac.issue = '□□시 시설관리과에서 추정가격 1억원 이상의 물품 구매(대형 제설차량 5대)를 규격·가격 통합 입찰로 처리하여 규격 경쟁을 가격 판단으로 흡수함으로써 기술력 낮은 최저가 업체와 계약한 사례.'
...
지방계약법 시행령 제18조에 따라 **추정가격 1억원 이상의 물품 구매**는 규격(기술 제안)을 먼저 평가하여 적격 업체를 선정한 뒤, 그 업체들 사이에서만 가격 경쟁을 시켜야 합니다.
...
규격가격 분리입찰은 공사에만 적용되지 않습니다. **물품 구매에서 추정가격 1억원 이상**이면 반드시 분리입찰을 시행해야 합니다. 이 점을 모르는 담당자가 많아 감사에서 자주 지적되는 항목입니다.

### 2. 분리입찰의 목적은 기술력 확보입니다

가격만으로 낙찰자를 결정하면 기술력이 낮은 업체가 최저가로 낙찰받아 실제 성능이 기대에 미치지 못하는 결과가 발생합니다. 추정가격 1억원 이상의 물품은 기관 운영에 중요한 장비인 경우가 많으므로, 기술 검증이 필수입니다.
...
나라장터에서 입찰 공고를 등록할 때 물품 추정가격이 1억원 이상이면 자동으로 분리입찰 여부를 확인하는 단계가 있습니다. 이를 건너뛰거나 통합 방식으로 변경하면 법령 위반입니다.
  LESSON

  ac.action_taken = '담당자 경고 처분, 계약 부적정 절차 판정, F사에 하자보수 개선 명령, 차기 물품 구매 시 규격가격 분리입찰 의무 이행 지시, 시설관리과 전 직원 입찰 방식 교육 실시'
  ac.checkpoints = [
    '물품 추정가격 1억원 이상 여부 확인',
    '1억원 이상이면 규격가격 분리입찰 방식 선택',
...
전문공사 기준 추정가격 2억원 초과는 일반경쟁입찰 대상입니다. 예산을 임의로 분할하여 이 기준 이하로 만드는 행위는 지방계약법 제9조(계약의 방법) 위반입니다.
...
지방계약법 제25조 라인에 따라 단가계약은 **계약서에 기재된 수량 한도 내에서만** 발주할 수 있습니다. 한도를 초과하면 다음 중 하나를 해야 합니다.
