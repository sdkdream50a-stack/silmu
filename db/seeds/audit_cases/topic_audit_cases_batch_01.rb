...
# ─────────────────────────────────────────────────────────────
# 1. 지연배상금 임의 면제(권한 없는 감면)
# ─────────────────────────────────────────────────────────────
AuditCase.find_or_initialize_by(slug: 'penalty-reduction-unauthorized').tap do |ac|
  ac.title = '지연배상금 임의 면제(권한 없는 감면) 사건'
  ac.category = '계약이행'
  ac.severity = '중대'
  ac.published = true
  ac.view_count = 0
  ac.topic_slug = 'penalty-reduction-procedure'
  ac.legal_basis = '지방계약법 제30조 (지연배상금 등), 지방계약법 시행령 제90조 (지연배상금), 행정안전부 예규 「지방자치단체 입찰 및 계약집행기준」 (책임 없는 사유 지체일수 제외 기준)'

  ac.issue = '△△군 농어촌 도로 포장공사(계약금액 14억원)에서 시공사가 40일 지체되어 지연배상금 2,800만원이 발생하였음에도, 계약담당자가 계약상대자의 책임 없는 사유(지방계약법 시행령 제90조 제1항 후단)에 해당하는지 객관적으로 검토·입증하지 않은 채 부과 의무가 있는 지연배상금 전액을 단독 전결로 임의 면제하여 지방재정 손실을 초래하였고, 감사에서 적발되어 중징계 처분이 내려짐.'
...
법령에 따라 △△군은 ◆◆건설에 지연배상금 2,800만원을 부과하여야 하였습니다.
...
지연배상금을 줄일 수 있는 **유일한 적법 경로**는 §90① 후단 — 계약상대자의 **책임 없는 사유**로 인정되는 지체일수를 산정에서 제외하는 것뿐이며, 이는 다음을 전제로 합니다:
...
**계약담당자 K씨:**
- 정직 3개월 (중징계)
- 변상 명령: 감면 처리한 지연배상금 2,800만원 전액

**◆◆건설(주):**
- 군에 지연배상금 2,800만원 납부 (분납 허가)
...
지방계약법 시행령 제90조 제1항은 지연배상금을 "납부하게 하여야 한다"고 규정한 **기속행위**입니다. 계약담당자가 임의로 감면·면제할 재량이 없고, 시행령에 '감면 심의위원회·단체장 승인' 같은 별도 감면 절차도 없습니다. 줄일 수 있는 유일한 길은 §90① 후단의 **책임 없는 사유 지체일수 제외**뿐이며, 이를 단독 전결로 면제하면 권한 없는 처분으로 감사 지적·변상 대상이 됩니다.

### 2. 책임 없는 사유는 객관적 증빙으로 입증해야 합니다
업체의 주장만으로는 책임 없는 사유가 인정되지 않습니다. 천재지변·파업 등 불가항력을 주장하는 경우, 해당 사유와 지체 발생 사이의 **인과관계**를 서류(기상청 자료, 언론 기사, 공문 등)로 입증해야 하며, 입증된 **해당 일수만** 지체일수에서 제외됩니다(전액 면제 아님).
...
공사 진행 중 지연 징후가 보이면 업체에 지연 원인 및 만회 대책을 **서면으로 제출**받아야 합니다. 이 과정에서 불가항력 사유가 있다면 공기 연장 절차(계약기간 연장)를 통해 처리하는 것이 적법하며, 이미 지체가 발생한 후 소급 감면하는 방식은 위법합니다.
  ...
  ac.action_taken = '계약담당자 정직 3개월 및 지연배상금 2,800만원 변상 명령, ◆◆건설 지연배상금 전액 납부, 지연배상금 면제·조정 시 책임 없는 사유 입증·합의제 검토 의무화, 최근 5년 지연배상금 면제·조정 내역 전수 재조사 실시'

  ac.checkpoints = [
    '지연배상금 부과는 기속행위 — 담당자 임의 감면·면제 권한 없음',
    '시행령 §90에는 별도 감면 심의·승인 절차 자체가 없음을 인지',
    '유일한 조정은 §90① 후단 책임 없는 사유 지체일수 제외',
    '책임 없는 사유와 지체 발생 간 인과관계 객관적 서면 입증 후 해당 일수만 제외',
    '공사 지연 발생 시 정당한 사유면 계약기간 연장으로 사전 처리(사후 임의 면제 금지)',
    '지연배상금 0원 처리 등 면제·조정은 입증자료 첨부 및 합의제 검토',
    '계약담당자 단독 전결 임의 면제 금지'
  ].to_json
...
AuditCase.find_or_create_by!(slug: 'private-contract-split-over-limit') do |ac|
  ...
  ac.legal_basis = '지방계약법 시행령 제25조 제1항 제5호 (소액 수의계약 — 세부 목은 계약유형·상대방 요건에 따라 확인), 지방계약법 시행령 제77조 (계약 분할 금지), 행정안전부 예규 제2023-24호 제5장 제2절 (계약 분할 금지 기준)'

  ac.issue = '□□도 ○○교육지원청에서 실내환경 개선사업(총사업비 1억 5천만원)을 수의계약 한도를 초과한다는 이유로 동일 업체와 3개 계약(각 5천만원)으로 분할 체결하여 지방계약법 시행령 제77조의 계약 분할 금지 조항을 위반하였고, 도 종합감사에서 적발되어 계약 담당자 징계 및 업체 입찰 참가자격 제한 처분을 받음.'
...
총사업비는 **1억 5천만원**입니다. 수의계약이 가능한 한도는 「지방계약법 시행령」 제25조제1항제5호에 따라 **계약유형(공사 / 물품·용역)과 상대방 요건(일반 업체·청년창업기업·소기업·소상공인 등)에 따라 달라지므로**, 이 사업이 어느 목에 해당하는지부터 확인해야 합니다. 이 사례에서 감사기관은 총사업비를 기준으로 경쟁입찰 대상이라고 판단하였습니다.
...
※ 이 조문은 **공사**에 적용됩니다. 물품·용역의 분할은 같은 영 제7조제2호(추정가격 합산)와 행정안전부 예규(집행기준 제1장 제1절 5.라)가 규율합니다.
...
동일한 목적·장소·시기의 공사나 용역을 공종별·장소별로 나누더라도, **전체 금액을 기준으로** 수의계약 한도(시행령 제25조제1항제5호 — 계약유형·상대방 요건에 따라 다름) 초과 여부를 판단합니다. 분할한 각 계약이 한도 미만이더라도 분할 자체가 위법입니다. "이 공종은 별도 공사"라는 논리는 동일 사업이라는 사실이 드러나는 순간 무너집니다.
...
AuditCase.find_or_create_by!(slug: 'qualification-failure-wrong-award') do |ac|
  ...
  ac.legal_basis = '지방계약법 시행령 제42조 (낙찰자 결정), 행정안전부 예규 「지방자치단체 입찰 및 계약 집행기준」 제4장 제3절 (적격심사 세부 기준)'
...
AuditCase.find_or_create_by!(slug: 'quote-collection-same-vendor-double') do |ac|
  ...
  ac.legal_basis = '지방계약법 시행령 제25조 제1항 제5호 (소액 수의계약 — 세부 목은 계약유형·상대방 요건에 따라 확인), 행정안전부 예규 제2023-24호 제5장 제3절 (2인 이상 견적서 징구 기준), 지방계약법 시행규칙 제43조 (견적서 제출 업체 독립성)'
