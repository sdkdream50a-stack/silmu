...
AuditCase.find_or_create_by!(slug: 'performance-guarantee-waiver-loss') do |ac|
  ...
  ac.legal_basis = '지방계약법 시행령 제56조 (이행보증금의 면제), 지방계약법 제15조의2 (계약보증금), 지방계약법 시행규칙 제66조 (이행보증금 면제 대상)'

  ac.issue = '○○시 공공시설 유지관리 용역(계약금액 8억원)에서 계약담당자가 지방계약법 시행령 제56조의 면제 요건(직전 2회 이상 성실 이행, 신용등급 BBB 이상 등)을 확인하지 않고 이행보증금(4천만원)을 면제해 주었고, 이후 해당 업체가 계약 이행을 포기하여 이행보증금 환수가 불가능해진 채 대체 계약 비용 등 총 7,200만원의 재정 손실이 발생함.'
...
지방계약법 시행령 제56조의 면제 요건은 한정적 열거입니다. 업체의 구두 주장은 증빙이 될 수 없으며, 반드시 신용평가서, 계약 이행 실적 증명서, 우수조달업체 인증서 등 **서면 증빙**을 징구하여야 합니다. 서류 없이 면제 처리하면 담당자가 전액 변상 책임을 질 수 있습니다.
...
AuditCase.find_or_create_by!(slug: 'private-contract-split-over-limit') do |ac|
  ...
  ac.legal_basis = '지방계약법 시행령 제25조 제1항 제1호 (소액 수의계약), 지방계약법 시행령 제25조 제6항 (계약 분할 금지), 행정안전부 예규 제2023-24호 제5장 제2절 (계약 분할 금지 기준)'

  ac.issue = '□□도 ○○교육지원청에서 실내환경 개선사업(총사업비 1억 5천만원)을 수의계약 한도액(5천만원)을 초과한다는 이유로 동일 업체와 3개 계약(각 5천만원)으로 분할 체결하여 지방계약법 시행령 제25조 제6항의 계약 분할 금지 조항을 위반하였고, 도 종합감사에서 적발되어 계약 담당자 징계 및 업체 입찰 참가자격 제한 처분을 받음.'
...
계약담당자는 징계 처분을, 계약 상대방은 입찰 참가자격 제한을 받습니다. 업체 입장에서도 수의계약의 편의를 누리는 대가로 향후 모든 공공입찰 자격이 제한될 수 있으므로, 분할 계약 요청에 응하지 않는 것이 업체에게도 유리합니다.
  ...
  ac.checkpoints = [
    ...
    '계약 분할 금지 조항(시행령 제25조 제6항) 위반 여부 계약 체결 전 자체 점검'
  ].to_json
