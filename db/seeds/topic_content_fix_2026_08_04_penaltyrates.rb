...
REPLACEMENTS = [
  ...
  # 선금을 예규 "위임" 사항으로 단정한 오류 — 지방회계법 제35조가 근거이고 예규는 세부기준이다(위임 관계 미입증).
  [ "기성금·준공금 지급 절차·지연이자율 등 세부사항은 시행령에, 선금은 「지방자치단체 입찰 및 계약집행기준」에 위임",
    "기성금·준공금 지급 절차·지연이자율 등 세부사항은 시행령에 위임 (선금의 근거·세부기준은 지방회계법 제35조와 행정안전부 예규 「지방자치단체 입찰 및 계약집행기준」에 따릅니다)" ]
].freeze
